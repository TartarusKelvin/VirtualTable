...
router.get("/view/:gameid", async (req,res)=> {
    ...
    try{
        let game = await Game.findById(req.params.gameid).populate("gm").populate("players").exec();
        res.render("games/viewgame", {game:game,canplay:game.players.some(e => e.secret === req.cookies.user_secret)});
    }catch{
        res.redirect("/game");
    }
})
